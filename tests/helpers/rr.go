...
const (
	...
	// statsTimeout bounds WaitStats; the reported state follows the consumer,
	// which lags a resume by a little.
	...
	// appear. Delivery from jetstream normally lands well inside a second.
...
// listener binds after the driver has dialed nats, so dialing it proves the
